    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        switch (session.getState()) {
            ...
            case MySqlSession.STATE_REMOVED:
                ...
                ctx.close();
                break;
            case MySqlSession.STATE_RESERVED:
                //标记删除状态信息，直接关闭链接吧。
                logger.warn("!!!状态[RESERVED]未处理信息:" + ByteBufUtil.prettyHexDump(buf));
                ctx.close();
                break;
            default:
                logger.warn("!!!状态[" + session.getState() + "]未处理信息:" + ByteBufUtil.prettyHexDump(buf));
                ctx.close();
