...
public class MySqlMaintenanceService {
    ...
    static boolean start() {
        if (STATE.compareAndSet(false, true)) {
            houseKeepingExecutor = new ScheduledThreadPoolExecutor(1, new ThreadFactoryBuilder().setNameFormat("mysql_house_keeping-%d").setDaemon(true).build(), new ThreadPoolExecutor.CallerRunsPolicy());
    ...
    static void scheduleHouseKeeping(MySqlService.HouseKeeper houseKeeper) {
        if (houseKeepingExecutor != null) {
            houseKeepingExecutor.scheduleWithFixedDelay(houseKeeper, 0L, 20, SECONDS);
        }
    }
